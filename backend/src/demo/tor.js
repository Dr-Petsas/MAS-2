...
/**
 * Schritt 1: Daten aufnehmen, Freischaltcode an die Handynummer schicken.
 *
 * @param {{vorname?:string, name?:string, praxis?:string, behandler?:string,
 *          website?:string, email?:string, handy?:string, ip?:string}} rein
 * @param {(auftrag:{an:string, text:string, absender:string}) => Promise<{ok:boolean}>} smsVersand
 * @returns {Promise<{ok:boolean, leadId?:string, fehler?:string, klartext?:string}>}
 */
export async function codeSenden(rein, smsVersand) {
  const handy = handyE164(rein?.handy);
  const name = text(rein?.name);
  const praxis = text(rein?.praxis);
  ...
  if (!name) return { ok: false, fehler: "name_fehlt", klartext: "Bitte trage deinen Namen ein." };
  if (!praxis) return { ok: false, fehler: "praxis_fehlt", klartext: "Bitte trage den Namen deiner Praxis ein — unter diesem Namen ruft Lisa später an." };
  if (!istEmail(email)) return { ok: false, fehler: "email_ungueltig", klartext: "Bitte prüfe die E-Mail-Adresse." };
  ...
  // Dieselbe Nummer soll denselben Lead weiterfuehren: sonst waere das
  // Kontingent durch mehrfaches Ausfuellen beliebig oft neu zu holen.
  ...
  const versand = await smsVersand({
    an: handy,
    // Kurz und ohne Link: Handys markieren Links von unbekannten Absendern
    // gern als Spam, und der Code ist alles, was gebraucht wird.
    text: `Pickadoc: Dein Freischaltcode für die Demo ist ${code}. Gültig 10 Minuten.`,
    absender: "Pickadoc",
  });
  if (!versand?.ok) {
    log.warn("demo.code.sms_fehlgeschlagen", { handy, fehler: versand?.error || "" });
    return { ok: false, fehler: "sms_fehlgeschlagen", klartext: "Die SMS ließ sich nicht senden. Bitte prüfe die Nummer." };
  }

  await ref.set({
    id: ref.id,
    vorname: text(rein?.vorname),
    name,
    praxis,
    behandler,
  ...
  log.info("demo.code.gesendet", { leadId: ref.id, handy, praxis });
